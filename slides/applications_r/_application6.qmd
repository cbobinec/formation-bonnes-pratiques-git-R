...
:::{.incremental}
3. Produire une courte note présentant le tableau des âges produit précédemment :
...
4. Générer le rapport avec le bouton `Render` et vérifier que l'*output* correspond à celui attendu.
:::
...
5. Dans `script.R`, nous avions créé un graphique `p.png`. 
...
6. A partir de la fonction suivante à mettre dans `R/functions.R`, 
...
7. Faire tourner le _pipeline_ (`tar_make`)
8. Pour être certain que votre note utilise la dernière version du _pipeline_, introduire
un `tar_make()` dans votre document à la suite du `library(targets)`
9. Cliquer sur le bouton `Render` pour mettre à jour votre note
:::
...
- [report.qmd](https://github.com/cbobinec/formation-bonnes-pratiques-git-R/tree/main/R/checkpoints/application6/report.qmd)
- [functions.R](https://github.com/cbobinec/formation-bonnes-pratiques-git-R/tree/main/R/checkpoints/application6/functions.R)
- [_targets.R](https://github.com/cbobinec/formation-bonnes-pratiques-git-R/tree/main/R/checkpoints/application6/_targets.R)
